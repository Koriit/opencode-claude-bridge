...
describe("patchNativeSkillVars — variable substitution for pre-existing cfg.skills.paths", () => {
  let tmp: { dir: string; cleanup: () => void }
  beforeEach(() => { tmp = makeTempDir() })
  afterEach(() => tmp.cleanup())

  function makeNativeSkill(name: string, body: string): string {
    const skillDir = path.join(tmp.dir, "native-skills", name)
    mkdirSync(skillDir, { recursive: true })
    writeFileSync(
      path.join(skillDir, "SKILL.md"),
  ...
  function asConfigWithPaths(paths: string[]): Config {
    return asConfig({ skills: { paths, urls: [] } })
  }

  test("skill without vars is left untouched — no cache copy written", async () => {
    const skillDir = makeNativeSkill("plain", "Just a plain skill.")
    const cacheRoot = path.join(tmp.dir, "cache")
    ...
    const mutable = cfg as unknown as { skills: { paths: string[] } }
    expect(mutable.skills.paths[0]).toBe(skillDir)
    expect(existsSync(cacheRoot)).toBe(false)
  })

  test("${CLAUDE_SKILL_DIR} is resolved to the skill's source directory", async () => {
    const skillDir = makeNativeSkill("dir-skill", "Run ${CLAUDE_SKILL_DIR}/setup.sh")
    const cacheRoot = path.join(tmp.dir, "cache")
    const cfg = asConfigWithPaths([skillDir])
